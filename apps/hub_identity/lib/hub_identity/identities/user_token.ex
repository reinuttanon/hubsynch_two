defmodule HubIdentity.Identities.UserToken do
  use Ecto.Schema
  import Ecto.Query
  ...
  schema "users_tokens" do
    ...
    belongs_to :user, HubIdentity.Identities.User
  ...
  def build_session_token(user) do
    ...
    {token, %HubIdentity.Identities.UserToken{token: token, context: "session", user_id: user.id}}
  end
  ...
  def build_email_token(address, context, client_service_id, user_id) do
    token = :crypto.strong_rand_bytes(@rand_size)
    hashed_token = :crypto.hash(@hash_algorithm, token)

    {Base.url_encode64(token, padding: false),
     %HubIdentity.Identities.UserToken{
       token: hashed_token,
       context: context,
       sent_to: address,
       user_id: user_id,
  ...
  The query returns the email found by the token.
  """
  def verify_register_email_token_query(token, context) do
      ...
      {:ok, decoded_token} ->
        ...
        query =
          from token in token_and_context_query(hashed_token, context),
            join: client_service in assoc(token, :client_service),
            where: token.inserted_at > ago(^days, "day"),
            select: {token.sent_to, client_service}
  ...
  def user_and_contexts_query(user, :all) do
    from t in HubIdentity.Identities.UserToken, where: t.user_id == ^user.id
  end
