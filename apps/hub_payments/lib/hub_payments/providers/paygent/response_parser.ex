defmodule HubPayments.Providers.Paygent.ResponseParser do
  ...
  def parse_capture_response({:ok, %HTTPoison.Response{status_code: 200, body: body}}) do
    decoded = Codepagex.to_string(body, "VENDORS/MICSFT/WINDOWS/CP932")
    require IEx
    IEx.pry()
  ...
  defp success([]), do: false

  defp success(["result=0" | _tail]), do: {:ok, "success"}

  defp success(["result=1" | _tail]), do: {:error, "failure result 1"}

  defp success(["result=7" | _tail]), do: {:error, "3d secure required"}

  defp success([_hd | tail]), do: success(tail)

  defp get_data([]), do: {:error, "no payment id"}
  ...
  defp get_data([_hd | tail]), do: get_data(tail)
end
